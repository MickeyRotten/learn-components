...
```js
new Function('register','esc','escA','fmt','ytVideoId','ytPlaylistId','ytThumb','ytIdBadge', text)(
  register, esc, escA, fmt, ytVideoId, ytPlaylistId, ytThumb, ytIdBadge
);
```
...
| `.ci-med` | `flex: 2` |
| `.ci-wide` | `flex: 3` |
| `.ci-prose` | Auto-growing textarea for prose fields (see Prose fields section) |
...
fmt(s)                   // esc() + prose markers + newlines → <br> (see below)
...
Fields holding teacher-written prose use `fmt(s)` instead of `esc(s)` in `gen()`, and are rendered in `ctrl()` as `<textarea class="ci ci-prose" rows="1">` rather than `<input>`.

`fmt()` escapes first, then converts markers and newlines:
...
Escaping runs before marker conversion, so typed markup stays inert. Markers are inserted by Ctrl+B / Ctrl+I / Ctrl+U, handled by a `keydown` listener on `#controls-panel` that calls `applyMarker()` and inserts via `execCommand('insertText')` so native Ctrl+Z still works. Pressing the same shortcut on already-wrapped text toggles it off.

`.ci-prose` fields auto-grow to fit their content via `autoGrow()` / `autoGrowAll()`, called on every `input` event and after each panel render.

**Rules:**
- Prose only — bodies, descriptions, list items, table cells. Never titles, headings, names, codes, dates, URLs, or pill labels: bold or a linebreak breaks those layouts.
- Inside a `<textarea>` body use `esc()`, never `escA()` — `escA` does not escape `<`.
- There is no escape for a literal `*` or `__`; paired occurrences will format.
...
Generated HTML should also **avoid `<div>` unless structurally necessary** (card grids, flex rows that genuinely need a block container). Nested divs paste badly into Learn and become opaque blocks in Moodle's WYSIWYG editor, so teachers can't click in and edit the text afterwards. Prefer semantic elements:

| Instead of | Use |
|---|---|
| Wrapper div around prose | `<p>` (styles go on the `<p>` itself) |
| Div per list item | `<ul>` / `<ol>` with `<li>` |
| Div-based rows and columns | `<table>` with `<tr>` / `<td>` |
| Div acting as a heading | `<h2>` / `<h3>` |
| Div for a pull quote or callout box | `<blockquote>` |
| Div wrapping an image or embed | `<figure>` + `<figcaption>` |
| Div used purely for inline styling | `<span>`, `<strong>`, `<em>` |

Both constraints apply to **generated output only**. `ctrl()` return values are the library app's own UI — never pasted into Moodle — and correctly use divs with the `.ctrl-*` classes.
