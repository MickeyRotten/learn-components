register({
  ...
  gen(st) {
    const exampleBlock = st.example.trim()
      ? `
  ...
  <div style="padding:18px 20px 20px 20px;">
    ...
    <p style="line-height:1.7;margin:0 0 14px 0;">${esc(st.definition)}</p>${exampleBlock}
  </div>
</div>`;
  },
  ctrl(st) {
    return `
    <div class="ctrl-header"><span class="ctrl-label">Definition Card</span></div>
    <div class="ctrl-rows">
      <div class="ctrl-row">
        <span style="font-size:11px;color:#666;flex-shrink:0;width:72px;font-family:var(--ui)">Term</span>
        <input class="ci ci-grow" type="text" value="${escA(st.term)}" data-f="term" data-i="0" placeholder="Term or concept">
      </div>
      ...
      <div class="ctrl-row" style="align-items:flex-start;">
        ...
        <textarea class="ci ci-grow" data-f="example" data-i="0" rows="2" style="resize:vertical;" placeholder="Leave empty to hide">${esc(st.example)}</textarea>
      </div>
    </div>`;
  },
  onInput(st, f, i, el) {
